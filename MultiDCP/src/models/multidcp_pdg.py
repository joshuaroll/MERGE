...
class MultiDCP(nn.Module):
    ...
    def forward(self, input_drug, input_gene, mask, input_cell_gex, input_pert_idose, epoch = 0):
        ...
        # --- gene embedding part: learnable per-gene embedding ---
        gene_indices = torch.arange(self.num_gene, device=drug_atom_embed.device)  # [num_gene]
        gene_embed = self.gene_index_embed(gene_indices)                            # [num_gene, gene_emb_dim]
        gene_embed = gene_embed.unsqueeze(0).repeat(num_batch, 1, 1)               # [batch, num_gene, gene_emb_dim]
