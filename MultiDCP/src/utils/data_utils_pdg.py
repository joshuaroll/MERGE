...
def read_data_binary(input_file, filter):
    ...
    return np.asarray(feature), np.asarray(labels, dtype=np.float32)
...
def read_data(input_file, filter=None):
    ...
    return np.asarray(feature), np.asarray(label, dtype=np.float32), cell_type
...
def transform_to_tensor_per_dataset_binary(feature, label, drug, device, basal_expression_file):
    ...
    for ft in feature:
        ...
        cell_id = ft[2]
        try:
            vec = basal_csv.loc[cell_id, :].to_numpy(dtype=np.float32)
        except KeyError:
        ...
        if use_pert_type:
            ...
            final_pert_type_feature.append(v.astype(np.float32))
        if use_pert_idose:
            v = np.zeros(len(pert_idose_set)); v[pert_idose_dict[ft[3]]] = 1
            final_pert_idose_feature.append(v.astype(np.float32))

    feature_dict = {'drug': np.asarray(drug_feature)}
    if use_pert_type:
        feature_dict['pert_type'] = torch.from_numpy(np.asarray(final_pert_type_feature, dtype=np.float32)).to(device)
    feature_dict['cell_id'] = torch.from_numpy(np.asarray(final_cell_id_feature, dtype=np.float32)).to(device)
    if use_pert_idose:
        feature_dict['pert_idose'] = torch.from_numpy(np.asarray(final_pert_idose_feature, dtype=np.float32)).to(device)
...
def transform_to_tensor_per_dataset_ehill(feature, label, drug, device, basal_expression_file):
    ...
    feature_dict = {
        ...
        'cell_id': torch.from_numpy(np.asarray(final_cell_id_feature, dtype=np.float32)).to(device),
    }
    if use_pert_idose:
        feature_dict['pert_idose'] = torch.from_numpy(np.asarray(final_pert_idose_feature, dtype=np.float32)).to(device)
