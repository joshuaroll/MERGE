...
def initialize_model_registry():
    ...
    model_param_registry['cell_id_input_dim'] = 10716
    model_param_registry['cell_id_emb_dim'] = 50
    model_param_registry['cell_decoder_dim'] = 10716 # autoencoder label's dimension
    model_param_registry['pert_idose_emb_dim'] = 4
    model_param_registry['hid_dim'] = 128
    model_param_registry['num_gene'] = 10716
...
def report_final_results(metrics_summary, ae = False, perturbed = False):
    ...
    if perturbed:
        ...
        # Only if the R^2 lists exist and are non-empty
        if metrics_summary['r2_top20_perturbed_dev']:
            print("Epoch %d got Perturbed R^2 (dev, top 20/40/80 DE genes): %.4f, %.4f, %.4f" %
                  (best_dev_epoch + 1,
                   metrics_summary['r2_top20_perturbed_dev'][best_dev_epoch],
                   metrics_summary['r2_top40_perturbed_dev'][best_dev_epoch],
                   metrics_summary['r2_top80_perturbed_dev'][best_dev_epoch]))
            print("Epoch %d got Perturbed R^2 (test w.r.t dev, top 20/40/80 DE genes): %.4f, %.4f, %.4f" %
                  (best_dev_epoch + 1,
                   metrics_summary['r2_top20_perturbed_test'][best_dev_epoch],
                   metrics_summary['r2_top40_perturbed_test'][best_dev_epoch],
                   metrics_summary['r2_top80_perturbed_test'][best_dev_epoch]))
        ...
        # --- NEW: R^2 summaries at best test epoch (test-only) ---
        if metrics_summary['r2_top20_perturbed_test']:
            print("Epoch %d got Perturbed R^2 on test set (top 20/40/80 DE genes): %.4f, %.4f, %.4f" %
                  (best_test_epoch + 1,
                   metrics_summary['r2_top20_perturbed_test'][best_test_epoch],
                   metrics_summary['r2_top40_perturbed_test'][best_test_epoch],
                   metrics_summary['r2_top80_perturbed_test'][best_test_epoch]))