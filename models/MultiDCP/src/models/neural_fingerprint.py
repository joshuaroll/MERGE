...
class NeuralFingerprint(nn.Module):
    ...
    def forward(self, drugs):
        ...
        fingerprint_atom = torch.zeros(batch_size, max_length, self.output_size).to(self.device)
        atom_activations = torch.zeros(num_atom, self.output_size).to(self.device)
        neighbor_by_degree = []
